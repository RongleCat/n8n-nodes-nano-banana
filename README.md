...
想让工作自动运转？关注微信公众号 **【曹工不加班】**，发送 **“n8n Nano Banana”** 获取 **授权码 (Auth Code)**，并解锁：
*   📚 **独家保姆级 n8n 实战教程**
*   🧩 **社区专享 VIP 节点**
*   💡 **高阶自动化工作流模板**
...
1.  **授权码 (Auth Code)**：**必填项**。关注公众号【曹工不加班】发送 “n8n Nano Banana” 获取。
2.  **连接类型**：
    *   **Official**：直接连接到 Google 的 Generative Language API。
    *   **OpenAI Compatible**：通过 OpenAI 兼容的端点连接。
3.  **API Key**：你的 Gemini API 密钥。
4.  **Base URL**：（可选）用于 OpenAI 兼容连接的自定义 Base URL。
